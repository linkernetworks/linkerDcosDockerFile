...
def extract_version_num(slaves_json):
    ...
    print('Mesos version: {} => {}'.format(version, num_str))
    return int(num_str)
...
def get_state_json(master_url, req_headers):
    ...
    if not version_num:
        ...
        slaves_json = get_url(urljoin(master_url, 'slaves'), req_headers)
        version_num = __master_versions[master_url] = extract_version_num(slaves_json)
    if not version_num or version_num >= 28:
        ...
        # 27 and before only have the reservation data in /state.json
        return get_url(urljoin(master_url, 'state.json'), req_headers)
...
def destroy_volume(slave, master_url, role, principal, req_headers={}):
    ...
    response = requests.post(req_url, headers=req_headers, data=data, verify=False)
...
def delete_zk_node(exhibitor_url, znode, req_headers={}):
    ...
    try:
        response = requests.delete(znode_url, headers=req_headers, timeout=5, verify=False)
    except requests.exceptions.Timeout:
        print('HTTP DELETE request timed out.')
        return False
    except requests.exceptions.ConnectionError as err:
        print('Network error:', err)
        return False
...
def delete_self_if_marathon(marathon_url):
...
def print_help(argv):
  ...
  [--auth_token=dcos_auth_tok]'''.format(argv[0], MASTER_DEFAULT, MARATHON_DEFAULT, EXHIBITOR_DEFAULT))
...
def main(argv):
    ...
    auth_token = ''
    try:
        opts, args = getopt.getopt(argv[1:], 'hvm:e:n:r:p:z:', [
            'help', 'verbose',
    ...
    if username and password:
        ...
        response = requests.post(req_url, verify=False, json={
            'uid': username,
            'password': password
        })
        if 200 <= response.status_code < 300:
    ...
    delete_self_if_marathon(marathon_url) # success, don't try again!
