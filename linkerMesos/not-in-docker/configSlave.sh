...
# echo paramters from environment to /etc/mesos/ /etc/mesos-slave
echo $MESOS_ZK > /etc/mesos/zk
env | grep MESOS_ | grep -v MESOS_ZK | grep -v MESOS_MASTER | grep -v MESOS_LOG_DIR | grep -v MESOS_QUORUM > mesosenvs
...
do
	...
	echo $mesosenvvalue > /etc/mesos-slave/$mesosparamfile
done
...
systemctl -H 127.0.0.1 enable mesos-slave
systemctl -H 127.0.0.1 start mesos-slave
